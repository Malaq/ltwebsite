...
sub zone_table {
	$zone_id = shift;
	my $zone_query =
		$dbh->prepare("SELECT NPC.NPC_ID, NPC.NPC_NAME, zo.ZONE_NAME, NPC.IS_BOSS " .
		"FROM NPC, NPC_IN_ZONE niz, ZONES zo " .
		"WHERE NPC.NPC_ID = niz.NPC_ID " .
		"  AND niz.ZONE_ID = zo.ZONE_ID " .
		"  AND niz.ZONE_ID = ? " .
		"ORDER BY NPC.IS_BOSS DESC, NPC.NPC_NAME;");

	$zone_query->bind_param(1, $zone_id);	
	...
	$zone_table = "$zone_table<TH><U><B>Boss</B></U></TH>";
	...
	while (my $row = $zone_query->fetchrow_hashref()) {
		$zone_table = "$zone_table<TR onMouseOver=\"this.className='highlight'\" onMouseOut=\"this.className='normal'\" onclick=\"location.href='npc.shtml?NPC_ID=$row->{NPC_ID}'\">";
		$zone_table = "$zone_table<TD>$row->{NPC_NAME}</TD>";
		$zone_table = "$zone_table<TD>$row->{IS_BOSS}</TD>";
...
print "<fieldset>";
print "<legend>Boss List:</A></legend>";
print zone_table($zone_id);
print "</fieldset>";
