...
sub bis_list {
	...
	if ($type eq "STAGE") 
	{
		...
		$SQL = 
		$dbh->prepare("SELECT BIS.LIST_DESCRIPTION, " .
			"	CL.CLASS_NAME, " .
			"       CS.SPEC_NAME, " .
			"       BT.TIER_NAME, " .
			"	 BT.TIER_DESC, " .
			"	 BT.TIER_ILEVEL_LOW, " .
			"	 BT.TIER_ILEVEL_HIGH " .
			"FROM BIS_ITEMS_STAGING BIS, " .
			"     CLASSES CL, " .
			"     CLASS_SPEC CS, " .
			"     BIS_TIERS BT " .
			"where BIS.CLASS_ID = CL.CLASS_ID " .
			"AND BIS.SPEC_ID = CS.SPEC_ID " .
			"AND BIS.TIER_ID = BT.TIER_ID " .
			"AND BIS.STAGE_SET_ID = ? " .
		...
		$table = "$table";
		$table = "${table}<B>BIS List Name:</B> $row->{LIST_DESCRIPTION}";

		$SQL = 
		$dbh->prepare("SELECT BIS.STAGING_ID, " .
			"       BIS.STAGE_SET_ID, " .
			"			 BIS.SLOT_ID, " .
			"			 BS.SLOT_NAME, " .
			" 			 BS.SLOT_SEQUENCE, " .
			"			 BIS.ITEM_ID, " .
			"			 IFNULL(IFNULL(IT.ITEM_NAME,BIS.ITEM_NAME_OVERRIDE),'<Enter Item Name>') as ITEM_NAME, " .
			"			 BIS.TIER_ID, " .
			"			 BT.TIER_DESC, " .
			"			 BIS.CLASS_ID, " .
			"			 CL.CLASS_NAME, " .
			...
			"			 BIS.ITEM_SUBTYPE, " .
			"			 BIS.ITEM_EQUIPLOC, " .
			"			 BIS.LIST_DESCRIPTION " .
			"FROM  BIS_SLOTS BS, " .
			"		 BIS_TIERS BT, " .
			"		 CLASSES CL, " .
			"		 CLASS_SPEC CS, " .
			"		BIS_ITEMS_STAGING BIS " .
			"		 LEFT JOIN ITEM AS IT " .
			"		 ON IT.ITEM_ID = BIS.ITEM_ID " .
			"WHERE BIS.SLOT_ID = BS.SLOT_ID " .
			"	AND BIS.TIER_ID = BT.TIER_ID " .
			"	AND BIS.CLASS_ID = CL.CLASS_ID " .
			"	AND BIS.SPEC_ID = CS.SPEC_ID " .
			"	AND BIS.STAGE_SET_ID = ? " .
			"ORDER BY BS.SLOT_SEQUENCE;");	
		...
		$table = "$table<TH><U><B>Item ID</B></U></TH>";
		$table = "$table<TH><U><B>Item Name</B></U></TH>";
		$table = "$table<TH><U><B>Item Link</B></U></TH>";
		$table = "$table<TH><U><B>Update</B></U></TH>";
		$table = "$table</TR>";
		$table = "$table</THEAD>";
		while (my $row = $SQL->fetchrow_hashref()) {
			$table = "$table<TR onMouseOver=\"this.className='highlight'\" onMouseOut=\"this.className='normal'\" >";
			$table = "$table<form name=\"row\" action=\"cgi-bin/updateStageItem.cgi\">";
			...
			$table = "$table<TD><input type=\"text\" name=\"ITEM_ID\" value=\"$row->{ITEM_ID}\" /></TD>";
			$table = "$table<TD><input type=\"text\" name=\"ITEM_NAME\" value=\"$row->{ITEM_NAME}\" /></TD>";
			$table = "$table<TD><a href=\"http://www.wowhead.com/?item=$row->{ITEM_ID}\">[$row->{ITEM_NAME}]</a></TD>";
			$table = "$table<TD><input type=\"submit\" name=\"update\" value=\"Update\" style=\"height: 20px; width: 60px\" /></TD>";
			$table = "$table</form>";
			$table = "$table</TR>";
		}
		...
		$table = "$table<input type=\"submit\" value=\"Save All\" style=\"height: 20px; width: 60px\" />";
...
sub bis_details {
	...
	# Details
	$SQL1 = 
		$dbh->prepare("SELECT BT.TIER_NAME, BT.TIER_DESC, BT.TIER_ILEVEL_LOW, BT.TIER_ILEVEL_HIGH, CL.CLASS_NAME, CS.SPEC_NAME " .
				"FROM BIS_TIERS BT, CLASSES CL, CLASS_SPEC CS " .
				"WHERE CS.SPEC_ID = ? " .
				"AND CL.CLASS_ID = ? " .
				"AND BT.TIER_ID = ? ;");

	$SQL1->bind_param(1, $spec_id);
	$SQL1->bind_param(2, $class_id);
	$SQL1->bind_param(3, $tier_id);

	$SQL1->execute() or die $dbh->errstr;

	my $row = $SQL1->fetchrow_hashref();
	my $tier_name = $row->{TIER_NAME};
	my $tier_desc = $row->{TIER_DESC};
	my $tier_ilevel_low = $row->{TIER_ILEVEL_LOW};
	my $tier_ilevel_high = $row->{TIER_ILEVEL_HIGH};
	my $class_name = $row->{CLASS_NAME};
	my $spec_name = $row->{SPEC_NAME};

	# Create List
	$SQL = 
		$dbh->prepare("SELECT BT.TIER_NAME, BS.SLOT_NAME, BS.SLOT_ID " .
				"FROM BIS_TIERS BT, BIS_SLOTS BS " .
				"WHERE BT.TIER_ID = ? " .
				"AND BS.DISPLAY = 'T' " .
				"ORDER BY BT.TIER_ID, BS.SLOT_SEQUENCE;");
	$SQL->bind_param(1, $tier_id);

	$SQL->execute() or die $dbh->errstr;
	
	$table = "${table}<BR><B>$spec_name $class_name</B><BR>${tier_name} - ${tier_desc}<BR>Item level range: <B>$tier_ilevel_low - $tier_ilevel_high</B><BR><BR>";
	$table = "$table<form name=\"details\" method=\"post\" action=\"cgi-bin/bisStage.cgi\" ONSUBMIT=\"return checkRequired(this)\" >";
	$table = "$table<input type=\"hidden\" name=\"TIER_ID\" value=\"$tier_id\" />";
	$table = "$table<input type=\"hidden\" name=\"CLASS_ID\" value=\"$class_id\" />";
	$table = "$table<input type=\"hidden\" name=\"SPEC_ID\" value=\"$spec_id\" />";
	$table = "${table}BIS List Name: <input type=\"text\" name=\"LIST_NAME\" required />";
	$table = "$table<BR><BR>";
	...
	$table = "$table<TH><U><B>Slot</B></U></TH>";
	$table = "$table<TH><U><B>Item ID</B></U></TH>";
	# Item name removed, will be asked for in the confirmation page if the item doesn't exist in the database.
#	$table = "$table<TH><U><B>Item Name</B></U></TH>";
#	$table = "$table<TH><U><B>Link</B></U></TH>";
	$table = "$table</TR>";
	$table = "$table</THEAD>";
	while (my $row = $SQL->fetchrow_hashref()) {
		$table = "$table<TR onMouseOver=\"this.className='highlight'\" onMouseOut=\"this.className='normal'\" >";
		$table = "$table<input type=\"hidden\" name=\"ITEM_SLOT\" value=\"$row->{SLOT_ID}\" />";


		$table = "$table<TD sorttable_customkey=\"$row->{SLOT_ID}\">$row->{SLOT_NAME}</TD>";
		$table = "$table<TD><input type=\"text\" name=\"ITEM_ID\" /></TD>";
		#$table = "$table<TD><input type=\"text\" name=\"ITEM_NAME\" /></TD>";
#		$table = "$table<TD></TD>";
		$table = "$table</TR>";
	}
	$table = "$table<TFOOT><TR><TD><input type=\"submit\" value=\"Confirm\" style=\"height: 20px; width: 60px\" /></TD></TR></TFOOT>";
	$table = "$table</TABLE>";
	$table = "$table</FORM>";
}
...
sub displayLists {
	...
	$table = "$table</THEAD>";
	while (my $row = $SQL->fetchrow_hashref()) {
		$table = "$table<TR onMouseOver=\"this.className='highlight'\" onMouseOut=\"this.className='normal'\" onclick=\"location.href='editbislists.shtml?CREATE_LIST=EDIT&BIS_LIST_ID=$row->{BIS_LIST_ID}'\">";
...
print "<fieldset>";
print "<legend><A HREF=\"bis.shtml\">BIS Lists:</A></legend>";

if (($create eq "T") and ($tier_id ne "") and ($class_id ne "") and ($spec_id ne "")) 
{
	print "<B>Please create your BIS list below:</B>";
	print "<BR>";
	print bis_details($create ,$tier_id, $class_id, $spec_id);

#	auditquery("SUCCESS","Create_Tier: $tier_name, Tier_Desc: $tier_desc, Low_Ilevel: $ilevel_low, High_Ilevel: $ilevel_high");

}
elsif (($create eq "") and ($tier_id ne "") and ($class_id ne "") and ($spec_id ne "")) 
{
	tier_title($tier_id);
	print "<B>Please choose your BIS list below:</B>";
	print "<BR>";
	print displayLists($tier_id, $class_id, $spec_id);
}
elsif ($create eq "STAGE")
{
	print "<B>Please confirm your BIS list below:</B>";
	print "<BR>";
	print bis_list("STAGE", $stage_set_id)
}
elsif ($create eq "EDIT")
{
	purgeOldStagedData();
	my $stage_set_id = getStageSetID();
	stageExistingData($stage_set_id, $bis_list_id);
	print "<B>Please edit your BIS list below:</B>";
	print "<BR>";
	print bis_list("STAGE", $stage_set_id)
}
else
{
	print "<B>There has been an error processing the page.  Please press back (<a href=\"bis.shtml\">or click here</a>) and choose a valid class/spec combination from a tier.  Error has been logged for user $audituser </B>";
	auditquery("ERROR","User attempted to access page incorrectly. Variables were, Create: $create, tier_id: $tier_id, class_id: $class_id, spec_id: $spec_id");
}
print "</fieldset>";
